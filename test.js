...
test('initial store', t => {
    const {summary} = m({store: {a: [1, 2, 3, 4, 5, null], b: 'bad'}});
    t.deepEqual(summary(), {a: {min: 1, max: 5, sum: 15, len: 5, avg: 3}});
    t.deepEqual(m({store: 'bad'}).summary(), {});
});

test('quantile', t => {
    const {collect, summary} = m({quantile: [0.25, 0.5, 0.75, 'bad']});
    collect('a', 1, 2, 3, 4);
    t.deepEqual(summary().a.quantile, {0.25: 1.25, 0.5: 2.5, 0.75: 3.75});
});

test('initial store with quantile', t => {
    const {collect, summary} = m({quantile: 0.5, store: {a: [1, 2]}});
    collect('a', 3, 4, 5);
    t.deepEqual(summary(), {a: {min: 1, max: 5, sum: 15, len: 5, avg: 3, quantile: {0.5: 3}}});
});

test('counters', t => {
    const {collect, summary} = m({counters: ['a', 'c'], store: {a: [1, 2]}});
    collect('a', 3, 4, 5);
    collect('b', 1, 2, 3, 4, 5);
    t.is(summary().a, 15);
